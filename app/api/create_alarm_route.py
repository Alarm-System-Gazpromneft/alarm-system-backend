...
def call_contacts(spisok_contacts, alarm, db, alarm_id):
    ...
    for contact in spisok_contacts_sorted:
        object_call = CallModule(host="0.0.0.0", port=8765)
        object_call.call(contact)
        ...
        for i in range(3):
            if object_call.dtmf_received(db.get_user_by_phone(contact).pin_code):
                object_call.speak(f"Прослушайте описание проблемы. {alarm.description}. Если вы готовы взять в работу данную проблему, нажмите 1, иначе, нажмите 0.")
